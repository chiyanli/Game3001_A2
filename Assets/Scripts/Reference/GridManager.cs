...
public class GridManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.M))
        {
            ...
            mines.Add(mineInst);
            ConnectGrid(); // Added for Lab 5. Inefficient at the moment. Recreates entire grid connections.
        }
        if (Input.GetKeyDown(KeyCode.C))
        {
            ...
            mines.Clear();
            ConnectGrid(); //Added for Lab 5.
        }
        if (Input.GetKeyDown(KeyCode.F)) //Find shortest Path.
        {
            // Get PathNode
            ...
            // Get PathNode under the planet in the graph.
            ...
            Path_Manager.Instance.GetShortestPath(start, goal);
        }       
    }
    ...
    private void BuildGrid()
    {
        ...
        for (int row = 0; row < rows; row++, rowPos--)
        {
            ...
            for (int col = 0; col < columns; col++, colPos++)
            {
                ...
                // New for Lab 5. Link a PathNode for each tile.
    ...
    public void ConnectGrid() // New for Lab 5, Changed to public.
    {
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < columns; col++)
            {
                ...
                tileScript.ResetNeighbourConnections(); // New for Lab 5.
                ...
                if (row > 0) // Set top neighbour if tile is not in top row.
                {
                    ...
                    {
                        //Connecting Tile to Tile. "Visual layer"
                        tileScript.SetNeighbourTile((int)NeighbourTile.TOP_TILE, grid[row - 1, col]);
                        // Connecting PathNode to PathNode. "Graph layer"
                        tileScript.Node.AddConnection(new PathConnection(tileScript.Node,
                            grid[row - 1, col].GetComponent<TileScript>().Node, 1f));
                        // Alternately for cost, instead of 1f do a distance check from tile to tile.
                    }
                }
                if (col < columns - 1) // Set right neighbour if tile is not in rightmost row.
                {
                    if (grid[row, col + 1].GetComponent<TileScript>().status != TileStatus.IMPASSABLE)
                    {
                        ...
                        tileScript.Node.AddConnection(new PathConnection(tileScript.Node,
                           grid[row, col + 1].GetComponent<TileScript>().Node, 1f));
                    }
                }
